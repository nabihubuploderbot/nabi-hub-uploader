...
نکتهٔ مهم: سرویس‌ها AsyncSession می‌گیرند نه session_factory؛
پس همیشه داخل `async with db.session_factory() as session` از آن‌ها استفاده می‌کنیم.
"""
import asyncio
...
async def cmd_start(
    ...
    # حالت تست اتصال: فقط تأیید می‌کنیم که آپدیت می‌رسد (برای عیب‌یابی)
    if cfg.DEBUG_ECHO:
        await message.answer(
            "✅ اتصال برقرار است! آپدیت /start دریافت شد.\n"
            f"chat_id شما: <code>{message.chat.id}</code> (این عدد را در MAIN_ADMIN_ID بگذارید)"
        )
    ...
    async with db.session_factory() as session:
        ...
        if code:
            await deliver_by_code(message, session, code, settings, user)
            return
...
async def deliver_by_code(message: Message, session, code: str, settings, user) -> None:
