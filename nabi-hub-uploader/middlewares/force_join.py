...
class ForceJoinMiddleware(BaseMiddleware):
    ...
    ✅ /start command IS checked — users must join before using deep links.
    Only CHECK_JOIN and CHECK_REACTION callbacks are skipped (for re-checking).
    """

    # Callbacks that should skip the force join check
    ...
    async def __call__(
        ...
        if isinstance(event, Message):
            ...
            user_id = event.from_user.id if event.from_user else None
            # Skip certain callbacks (for re-checking)
            if event.data:
        ...
        # ── User hasn't passed force join — block and show message ──
